...
FenClient::FenClient()
{
    infoServ = new QHBoxLayout;
    infoClient = new QHBoxLayout;
    layout = new QVBoxLayout;

    ip = new QLabel;
    ip->setText("Ip du serveur: ");
    port = new QLabel;
    port->setText("Port du serveur: ");
    pseudoLabel = new QLabel;
    ...
    infoServ->addWidget(ip);
    infoServ->addWidget(serveurIP);
    infoServ->addWidget(port);
    infoServ->addWidget(serveurPort);
    infoServ->addWidget(boutonConnexion);

    infoClient->addWidget(pseudoLabel);
    infoClient->addWidget(pseudo);
    infoClient->addWidget(messageLabel);
    infoClient->addWidget(message);
    infoClient->addWidget(boutonEnvoyer);

    layout->addLayout(infoServ);
    layout->addWidget(listeMessages);
    layout->addLayout(infoClient);
    setLayout(layout);
    ...
    connect(boutonConnexion, SIGNAL(clicked(bool)), this, SLOT(on_boutonConnexion_clicked()));
    connect(boutonEnvoyer, SIGNAL(clicked(bool)), this, SLOT(on_boutonEnvoyer_clicked()));
    connect(message, SIGNAL(returnPressed()), this, SLOT(on_boutonEnvoyer_clicked()));
